...
RUN apk add --no-cache curl jq \
    ...
    && curl -sSL "https://github.com/just-containers/s6-overlay/releases/download/v${S6_RELEASE}/s6-overlay-symlinks-noarch.tar.xz" -o "/tmp/s6-overlay-symlinks-noarch.tar.xz" \
    && curl -sSL "https://github.com/just-containers/s6-overlay/releases/download/v${S6_RELEASE}/s6-overlay-symlinks-noarch.tar.xz.sha256" -o "/tmp/s6-overlay-symlinks-noarch.tar.xz.sha256" \
        && curl -sSL "https://github.com/just-containers/s6-overlay/releases/download/v${S6_RELEASE}/s6-overlay-symlinks-arch.tar.xz" -o "/tmp/s6-overlay-symlinks-arch.tar.xz" \
    && curl -sSL "https://github.com/just-containers/s6-overlay/releases/download/v${S6_RELEASE}/s6-overlay-symlinks-arch.tar.xz.sha256" -o "/tmp/s6-overlay-symlinks-arch.tar.xz.sha256" \
    && cd /tmp \
    && sha256sum -c s6-overlay-noarch.tar.xz.sha256 \
    && sha256sum -c s6-overlay-${S6_PLATFORM}.tar.xz.sha256 \
    && sha256sum -c s6-overlay-symlinks-noarch.tar.xz.sha256 \
    && sha256sum -c s6-overlay-symlinks-arch.tar.xz.sha256 \
    && mkdir -p /s6/root \
    && tar -C /s6/root -Jxpf /tmp/s6-overlay-noarch.tar.xz \
    && tar -C /s6/root -Jxpf /tmp/s6-overlay-${S6_PLATFORM}.tar.xz \
    && tar -C /s6/root -Jxpf /tmp/s6-overlay-symlinks-noarch.tar.xz \
    && tar -C /s6/root -Jxpf /tmp/s6-overlay-symlinks-arch.tar.xz
